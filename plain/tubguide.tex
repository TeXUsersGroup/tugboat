...
%%     copyright  = "Copyright 1989, 1992, 2006, 2012-2026 TeX Users Group.
...
%%                   TUGboat vol.10, no.3, November 1989.",
%%     }
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%not needed \StretchyNinePointSpacing
%not needed \StretchyEightPointSpacing
...
\pageno=1
\def\rtitlex{\def\tubfont{\tenpoint\rm}\TUB{} Authors' Guide, \today}
\def\midrtitle{}

\title *\TUB\/ authors' guide*

\author  * Barbara Beeton, Karl Berry *
...
\vfootnote{}{Revised March 1992, May 2006, May 2012, September 2016, and
subsequently; the original appeared in
\tubissue 10(3), November 1989.}

\article

We hope this article can serve as a reasonable guide to preparation of
plain \TeX\ manuscripts for \TUB. Suggestions and comments are quite
welcome at the address listed below.

\TUB\ accepts submissions in plain \TeX, \LaTeX, and other \TeX\
formats, all with much the same output, and similar input conventions
where reasonable. This document describes the \plain-based macros for
\TUB. On \CTAN, it is
\tbsurl{ctan.org/pkg/url/tugboat-plain}.\footnote{}{The \LaTeX\ style
for \TUB\ is the file |ltugboat.sty|; on \CTAN:
\tbsurl{ctan.org/pkg/tugboat}.} We conclude with some general
suggestions to help make the lives of those on the receiving end of (any
kind of) electronic copy a little easier.


\head * The \plain-based macros: {\tt tugboat.sty} *

The macros are contained in the file |tugboat.sty|.\footnote{}{%
1) A file |tugproc.sty| is also distributed, but no longer used.
2) A file |tugboat.cmn| was distributed until 2026 (version 1.33); its
contents are now incorporated in |tugboat.sty|.}
%3) |tugboat.cmn| was originally named {\tt tugboat.com}, but that notation
%was in conflict with conventions of \acro{MS-DOS} and other operating
%systems.}

\subhead * General description of tags *

We attempt wherever possible to tag the various elements of \TUB\/
articles in a ``generic'' way, modified in some respects by convenience.
Thus, the tags whose use we strongly encourage are the higher-level tags
that mark the logical document structure. Below these are formatting
macros that we recognize may be essential for certain applications. Both
sorts of tags are described in the following article.

Generally, to ``mark up'' some input |<foo>|, a tag |\xxx| will precede
|<foo>| and |\endxxx| will follow, as in: |\xxx <foo>\endxxx|.

Optional commands follow tags and are enclosed in
...
For convenience, we also allow |*| as a delimiter for the
...
Generally authors are listed separately in the signature. In cases where
authors and addresses are to be combined, one may use |\signature{...}|
and |\signaturemark| with some or all of
||
...
it has been left intact for posterity.  The correct address
...
The \TUB\ footnote macros are firmly based on those of plain \TeX, with
adjustments to the formatting. For example, \TUB\ footnotes are typeset
in the 8pt font size (using \cs{eightpoint}).

The \TUB\ footnote macros do have one feature beyond plain \TeX: support
for automatically-numbered footnotes.  This is done by leaving the first
argument to \cs{footnote} empty:

||
\footnote{}{This footnote will be
automatically numbered.}
||
Each \cs{footnote} command increments the (\TUB-specific) counter
\cs{footnotenumber}, should references or other variations of footnote
numbering be desired.

Before this feature was implemented (September 2026, version~1.33), this
construct was commonly used for numbered footnotes, hard-coding the number:
...
For a footnote without a number, \plain's \cs{vfootnote} is used. On
...
\subhead * Lists *
Perhaps not surprisingly, |\list| marks the beginning of a list.  A list
...
Tags for the items default to the |\bullet|
...
you get what you type, no escaping by default).  In addition to the long form
...
\endverbatim
||
form. The construct \verbinline||...||\endverbatim can be a good way to
...
gained as a result are useful.  If |\smallverbdisplay| is defined
to be a no-op, it will be in the usual ten-point.  (This change was
introduced in 2012.)

\subsubhead * Reading files verbatim. *
...
\endverbatim
||
will incorporate the contents of file |foo.tex| in the listing before
the text between |\verbatim| and |\endverbatim|.  The shortened form
||\verbfile{foo.tex}\endverbatim|| accomplishes the above in the case
that the text is empty.  While the code around the data, |foo.tex|,
...
We often rearrange code supplied to us so that it fits in the narrow
measure of \TUB's two-column format, and we sometimes make corrections
to macro sets. Since errors can (and do\Dash we aren't perfect) creep in
with these modifications, we try to use the above technique to maintain
consistency between the listing published in \TUB\/ and the underlying
macros used for examples.
...
\subsubhead * Verbatim options. *

\leavevmode|\verbatim| scans forward to |\endverbatim| (a 12-token sequence
...
verbatim mode. The contents of token register |\everyverbatim| is
scanned after the defaults of verbatim mode have been set. For example,
the present article uses
||\everyverbatim{\enablemetacode}||
so that the
input |[\disablemetacode]<foo>| inside verbatim will output like
|\meta{foo}| (\meta{foo}), i.e., with math angle brackets and text in
italics, by
making |[\disablemetacode]<| active. To then typeset an actual
verbatim |[\disablemetacode]<|, as in the sentence here, it's necessary to
disable the catcode change:
\verbinline|\endverbatim|[\disablemetacode\lastoption][\disablemetacode]<|\verbinline|\endverbatim.
...
distracting rather than helpful. To keep things simple, there are no
display options.
...
bottom-insertions, respectively.

Here we recommend that the long-form terminator be used ({\it not\/} the
|*...*| form). One can think of the information ``passed'' as being
``long'' in the sense of possibly containing paragraphs, this being a
mnemonic device only. The primary reason for the recommendation is that
one is (in some sense, maybe) more likely to encounter a rogue |*| in
longer text than in shorter text and hence more likely to encounter a
surprising result due to a macro stopping short at the wrong |*|.
...
A primitive |\caption{...}| option is available to |\ulap| (i.e., lap
upward) its argument into the figure space, and the argument is typeset
in \cs{ninepoint}. The formatting of the caption is otherwise left to
the user, including the label. For example, the code:
\verbatim[\outputtofile{code.vbm}]
\figure[\top]
  [\caption{\centerline{%
...
Figures spanning columns at the top and bottom of a page are
unfortunately supported only on the first page of an article.
|\twocolfigure| (terminated by |\endfigure|) starts up such a figure and
currently {\it must\/} occur before any material has been typeset on the
first page (i.e., {\it before\/} |\article|).
(The workaround is to avoid macros and position the desired figure as a
box, positioning manually.)
...
The present manual includes some text in a smaller font (9pt) and set
off by vertical space above and below. These blocks are created by the
|\subtext| command. We don't recommend using it for articles to be
published in \TUB, but include this description for completeness.

\subtext This is an example of such ``subtext''. There is no
|\subsubtext| or any other level.
\endtext

\subhead * Command summary *
...
Definitions of many commonly used abbreviations are provided. Please use
these whenever possible rather than creating your own. We will add to
the list as necessary.

\list
\item |\acro| sets an all-caps argument in caps one size smaller than
the surrounding text. Compare CTAN (full size), \CTAN{} (|\acro{CTAN}|)
and {\smc ctan} (small caps). The argument must always be all caps,
perhaps with hyphens, slashes, and/or numbers, but never a lowercase
letter. We use |\acro| consistently in \TUB, both in our macros and when
editing articles, with the exception of bibliographies.

\item |\cs{foo}| typesets |\foo|, just like \verbinline|\foo|\endverbatim.
Since |\cs| is the usual \TUB\ \LaTeX\ convention, we support it here too.

\item |\Dash| is an em-dash with surrounding thin\-spaces, our preferred style.

\item |\hyph| typesets an explicit hyphen and allows hyphenation in the
following word (unlike a |-| character).

\item |\meta{foo}| typesets \meta{foo} in italics and inside angle
brackets. If your article uses meta-variables pervasively, you can use
the command \cs{enablemetacode} to allow the input
|[\disablemetacode]<foo>| to do the
same thing; this makes the |[\disablemetacode]<| character globally active.
\cs{disablemetacode} undoes this.

\item |\slash| is a / allowing a line break after.

\item |\titleref{Foo: A Book}| typesets the argument in the
slanted font (\cs{sl}) and with \cs{frenchspacing}. For example, there's
no extra space after the |:| here: \titleref{Foo: A Book}. It's intended
as markup for the title of any ``book like'' object, rather than
explicitly using \cs{it}, \cs{sl}, etc.

\item Various |\*lap|\,s (|\ulap|, |\dlap|, |\xlap|,
...
Plain \TUB\ redefines \cs{strut} to be different than plain \TeX, namely
with the height and depth of the parenthesis in the current font;
\cs{strutt} is defined to be nearly the same as the plain \TeX\
\cs{strut}, with the height of the parenthesis and the depth as
necessary to make the height+depth be \cs{normalbaselineskip}.

\item |\makeatletter| |\catcode|s the |@| for internal control-sequences.
...
late in the production process. By default, |\xrefto| is defined to
simply typeset ``???'; not to worry.
...
A wider perspective in the matter of naming macros can prevent problems
that occur when definitions are overwritten. The names of control
sequences used in \plain, \LaTeX, and \AmSTeX\ are documented and
authors should avoid using them for other purposes. It is also wise to
avoid commonly used names such as |\temp|, |\result|, |\1|, and |\mac|
in presenting code that might be cribbed by other users.

Both authors and readers are in need of small macros to do little
tricks, and they often try suggestions brought forth in \TUB. A little
extra effort in making new macro names consistent, rather than in
conflict, with the macros in wide distribution and in making them robust
...
\list
\item plain: \tbsurl{ctan.org/pkg/tugboat-plain}
\item \LaTeX: \tbsurl{ctan.org/pkg/tugboat}
\endlist

Please address all electronic correspondence to the \TUB{} maildrop:

\list
\item \tbmailto{TUGboat@tug.org}
\endlist

Mail to personal addresses is can go unseen when life intervenes, so
please use the generic address whenever appropriate.
